function [W_fuel,W_wing,counter] = MDA_coordinator(X, W_fuel_c, W_wing_c)
...
%Defining the design variables X
    ...
    while abs((W_fuel - W_fuel_c))/W_fuel >error || abs(W_wing - W_wing_c)/W_wing > error
        ...
        [LD,~,~,~] = Aero(Span,Area,TPR,QC_Sweep,UCST,LCST,W_wing_c,W_fuel_c,0);
        data = Loads(Span,Area,TPR,QC_Sweep,UCST,LCST,W_wing_c,W_fuel_c,H_cruise,0);
        W_wing = Structures(W_fuel_c,Span,Area,TPR,QC_Sweep,UCST,LCST,W_wing_c,spar_front,spar_back,0);
        W_fuel = Performance(M_cruise,H_cruise,W_wing_c,W_fuel_c,LD,0);
    ...
    fid = fopen("CST_show.txt","a");
    fprintf(fid, '%f\t', [UCST LCST]);
    fprintf(fid, '\n');
    fclose(fid);
